...
class WebPageView extends React.Component {
  constructor(props) {
    ...
    const {
      about, feature, locales,
    } = props

    this.lighthouses = (about.objectIn || []).filter(action => action['@type'] === 'LighthouseAction') || []
    this.likes = (about.objectIn || []).filter(action => action['@type'] === 'LikeAction') || []

    this.sortedNames = about.name ? [
    ...
    this.sortedDescriptions = about.description ? [
    ...
    this.centroid = (this.country && centroids[this.country]) || null
  }

  componentDidMount() {
    if (document.getElementById('static-map')) {
      this.L = require('leaflet')

      this.map = this.L.map('static-map');
      ...
      if (this.geometry) {
        ...
        const self = this
        this.pointGeojson = this.L.geoJSON(feature, {
          pointToLayer: function (feat, latlng) {
            const marker = self.L.marker(latlng, {
              icon: self.L.icon({
                iconUrl: markerIcon,
                iconSize: [25, 41],
                iconAnchor: [12, 41],
        ...
        this.pointGeojson.addTo(this.map)
        this.map.setView([this.geometry.coordinates[1], this.geometry.coordinates[0]], 4)
  ...
  render() {
    const {
      about, expandAll, isLiveEvent, moment, schema, translate, user, view, _self
    } = this.props
        ...
        <div className="row stack-700 stack-gutter-2em">
          ...
          <div className="col two-third">
            ...
            >
              ...
              >
                {about.description ? (
                  Object.keys(about.description).length > 1 ? (
                    <Tabs>
                      {this.sortedDescriptions.map(lang => (
                      ...
                      <TabList>
                        {this.sortedDescriptions.map((lang, i) => (
                          <Tab key={`tab-${lang}`}>
              ...
              {this.lighthouses.length > 0 && about['@id'] && (
          ...
          <aside className="col one-third">
            ...
            {(this.lighthouses.length > 0 || this.likes.length > 0 || about['@type'] === 'Policy') && (
